...
import numpy
"""numpy.Error.setMode(dividebyzero="raise")"""
...
def crossProduct( set1, set2):
    ...
    vy = set2[:,1]
    vz = set2[:,2]
    result = numpy.zeros( (len(set1),3), set1.typecode())
...
def crossProduct4( set1, set2 ):
    ...
    vy = set2[:,1]
    vz = set2[:,2]
    result = numpy.zeros( (len(set1),4), set1.typecode())
    result[:,0] = (uy*vz)-(uz*vy)
    result[:,1] = (uz*vx)-(ux*vz)
    result[:,2] = (ux*vy)-(uy*vx)
    return result
...
def magnitude( vectors ):
    ...
    if not (len(numpy.shape(vectors))==2 and numpy.shape(vectors)[1] in (3,4)):
        vectors = numpy.reshape( vectors, (-1,3))
    vectors = vectors*vectors
    # should just use sum?
    result = vectors[:,0]
    numpy.add( result, vectors[:,1], result )
    numpy.add( result, vectors[:,2], result )
...
def normalise( vectors ):
    ...
    return numpy.divide( vectors, mags)
